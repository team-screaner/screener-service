...
Go-сервис и веб-интерфейс для матриц компетенций, подтверждающих фактов, самооценки и планов развития. PostgreSQL хранит связи и неизменяемые снимки; контракт API — [`api/openapi.yaml`](api/openapi.yaml). Интерфейс встроен в Go-бинарник и доступен на `/`.
...
Откройте [веб-интерфейс](http://127.0.0.1:18081/) для текущего локального запуска на `18081` (стандартный порт — `8080`). Создайте аккаунт, выберите матрицу в каталоге и текущий/целевой уровни. Затем добавьте факт с привязкой к требованию, сохраните самооценку и составьте план развития. Поддерживаются просмотр матрицы, обзор прогресса, radar, принятие агентских фактов, XLSX preview/import/publish/export и создание/отзыв токенов агентов.

Сессия интерфейса хранится в `sessionStorage` текущей вкладки; выход отзывает её на сервере. Черновик самооценки хранится только в памяти до сохранения. Редактор корпоративных матриц, управление организациями и разбор upstream-изменений доступны пока через API; текущий интерфейс покрывает личный сценарий развития. Подробнее — [frontend.md](docs/frontend.md).
...
make test-web          # JS-модели, API-клиент и синтаксис; Node 22+
npm ci --prefix web    # happy-dom нужен только для сквозного теста
SCREENER_URL=http://127.0.0.1:18081 make test-web-live
```
