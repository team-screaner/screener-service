...
Swagger UI доступен на `/docs/` (локально: [http://127.0.0.1:18081/docs/](http://127.0.0.1:18081/docs/), при стандартном порте Compose — `8080`). Контракт: `/openapi.json`. Выполните register/login и вставьте полученный `token` в **Authorize** без `Bearer`; затем используйте **Try it out**. Для команд заполните обязательный `Idempotency-Key`. Документация публична, API сохраняет обычные проверки доступа. UI встроен в бинарник и работает без CDN; токен не сохраняется после перезагрузки. Версия и происхождение файлов: [swagger-ui.md](docs/swagger-ui.md).
