...
Final local checks: `make test`, `make test-integration` (race/shuffle), 13 frontend tests, live forms against the rebuilt Docker service, and standards lint (0 issues) passed. Independent generated-client acceptance coverage is updated in `screener-service-autotests`. Seventeen review-access/list/write, assessment, shared-evidence and export SQL statements were inspected with `EXPLAIN (ANALYZE, BUFFERS)` on synthetic development records inside rollback transactions; these checks are not a production-scale performance benchmark.

Personal XLSX export keeps the latest **self** assessment in the existing Assessment sheet. A regression test first reproduced a newer manager snapshot replacing that sheet; the export now filters by type and uses the same latest-self ordering as the radar. Manager comparisons remain available in the UI/API.
